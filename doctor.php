...
if(isset($_POST[submit]))
{
	...
	{
			$sql ="UPDATE doctor SET doctorname='$_POST[doctorname]',mobileno='$_POST[mobilenumber]',departmentid='$_POST[select3]',loginid='$_POST[loginid]',password='$_POST[password]',status='$_POST[select]',education='$_POST[education]',experience='$_POST[experience]',consultancy_charge='$_POST[consultancy_charge]',email='$_POST[email]' WHERE doctorid='$_GET[editid]'";
		...
		else
		{
			echo mysqli_error($con);
				...
				//update user table
				$sql = "UPDATE users SET loginid='$_POST[loginid]',password='$_POST[password]',type='doctor',email='$_POST[email]' WHERE deletID='$_GET[editid]'";
				if ($qsql = mysqli_query($con, $sql)) {
	...
	{
	$sql ="INSERT INTO doctor(doctorname,mobileno,departmentid,loginid,password,status,education,experience,consultancy_charge,email) values('$_POST[doctorname]','$_POST[mobilenumber]','$_POST[select3]','$_POST[loginid]','$_POST[password]','Active','$_POST[education]','$_POST[experience]','$_POST[consultancy_charge]','$_POST[email]')";
	if($qsql = mysqli_query($con,$sql))
	{
	...
	else
	{
		echo mysqli_error($con);
			...
			/////select data from doctor database to get deleteid of user tablae
			$sql = "SELECT * FROM doctor WHERE loginid='$_POST[loginid]'  ";
			$qsql = mysqli_query($con, $sql);
			$getDeleteIDfromUser = mysqli_fetch_array($qsql);
	
			$getdoctorid = $getDeleteIDfromUser[doctorid];

		//insert data to user tables
		$sql ="INSERT INTO users(loginid,type,password,email,deletID) values('$_POST[loginid]','doctor','$_POST[password]','$_POST[email]','$getdoctorid')";
		if($qsql = mysqli_query($con,$sql))
		{
			echo "<div class='alert alert-success'>
